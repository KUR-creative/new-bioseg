...
%
% Update:
%   2018-12-15: Incorrect calculation for FN is fixed. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% listOfGTObjs = unique(G);                                       %all GTs
% listOfGTObjs(listOfGTObjs == 0) = [];                           %delete background

% listOfGTObjsWithCorrespondingSegObj = tempMat(:,2);             %objs that were matched with an instance
% listOfGTObjsWithCorrespondingSegObj(listOfGTObjsWithCorrespondingSegObj == 0) = []; %if background, then delete
% listOfGTObjsWithOutCorrespondingSegObj = setdiff(listOfGTObjs,listOfGTObjsWithCorrespondingSegObj); %All - withTP
% 
% %display(["GT with Seg " num2str(length(listOfGTObjsWithCorrespondingSegObj))]);
